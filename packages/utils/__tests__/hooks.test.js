...
describe('@morjs/utils - hooks.test.js', () => {
  describe('tsTransformerFactory', () => {
    ...
    it('should call visitor for each node and handle array result from visitor', () => {
      const visitor = jest.fn().mockImplementationOnce(() => [
        { kind: ts.SyntaxKind.Identifier, pos: 0, end: 1 }, // 模拟 visitor 返回一个包含两个节点的数组
        { kind: ts.SyntaxKind.StringLiteral, pos: 2, end: 3 },
      ]);
      const transformerFactory = tsTransformerFactory(visitor);
      const ctx = {};
      const sourceFile = {};

      const visitNodeSpy = jest.spyOn(ts, 'visitNode').mockImplementation((node, visit) => {
        visit(node);
        return node;
      });
      const visitEachChildSpy = jest.spyOn(ts, 'visitEachChild').mockImplementation((item, childVisitor) => childVisitor(item));

      transformerFactory(ctx)(sourceFile);

      expect(visitNodeSpy).toHaveBeenCalledWith(sourceFile, expect.any(Function));
      expect(visitor).toHaveBeenCalled();

      // 验证对数组中每个元素都调用了 ts.visitEachChild
      expect(visitEachChildSpy).toHaveBeenCalledTimes(2); 

      visitNodeSpy.mockRestore();
      visitEachChildSpy.mockRestore();
    })
  })
